...
import { requestNotifications } from "./src/features";
import type { CardModel, CategoryModel, GameState } from "./src/models";
import {
    buttonGridClass,
    flexContainer,
    flexContainerItem,
...
function App() {
    ...
    const handleDateChange = (date: string) => async (_e: MouseEvent) => {
        ...
        const response = await sw.puzzle[":date"].$get({
            param: { date },
        });
        if (!response.ok) {
            setGameState(null);
            ...
            <div class={flexContainerItem}>
                ...
                <div class={buttonGridClass}>
                    ...
                    <button
                        type="button"
                        onClick={async () => {
                            console.log(await (await sw.index.$get())?.text());
                        }}
                    >
                        POKE
                    </button>
                    ...
                    <button
                        type="button"
                        onClick={async () => {
                            console.log(await (await sw.ping.$get())?.text());
                        }}
                    >
                        PING
                    </button>
